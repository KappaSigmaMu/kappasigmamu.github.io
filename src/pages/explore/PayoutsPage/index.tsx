...
const PayoutsPage = (): JSX.Element => {
  ...
  const state = useChainQuery(() => api ? getSocietyMembersWithPayouts(api, graceStrikes) : undefined, [api, graceStrikes, trigger])
  if (state.error) return <ChainError error={state.error} onRetry={state.refetch} />
  if (!state.data) return <LoadingSpinner />
  return <PayoutsList members={state.data} activeAccount={activeAccount} handleUpdate={() => setTrigger((previous) => !previous)} />
}
