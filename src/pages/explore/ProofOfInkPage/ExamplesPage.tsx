  ...
  return (
    <>
      ...
      <StyledDataRow>
        ...
        <Col lg={2} className="text-center text-lg-start">
          <AccountIndex accountId={head} callback={setIndex} />
        </Col>
...
function DesignKusama({ accountId, accountIndex }: { accountId: AccountId; accountIndex: string }) {
  ...
  useEffect(() => {
    ...
    if (ctx) draw(ctx, accountId, accountIndex)
  }, [accountId, accountIndex])
  return (
    <canvas
      height={SIZE * rows + PADD}
      ref={canvasRef}
      style={{ display: 'block', margin: '0 auto', backgroundColor: 'white', letterSpacing: '0.015em', width: '100%' }}
      width={SIZE * 3 + PADD * 2}
    />
  )
}
